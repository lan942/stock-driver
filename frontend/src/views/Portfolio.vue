...
const loadOverview = async () => {
  try {
    const response = await stockAPI.getPortfolioOverview()
    overview.value = response.data
  } catch (error) {
    console.error('加载持仓总览失败:', error)
  }
}

const loadHoldings = async () => {
  try {
    const response = await stockAPI.getHoldings()
...
const submitEditHolding = async () => {
  ...
  try {
    await stockAPI.updateHolding(editHoldingForm.id, {
...
const clearAllTransactions = async () => {
  try {
    ...
    await stockAPI.clearTransactions()
...
const submitCashUpdate = async () => {
  ...
  try {
    await stockAPI.updateCashBalance(cashForm.amount)
